...
p_boxes = [0x243F6A88, 0x85A308D3, 0x13198A2E, 0x03707344,
...
def round_func(xl):
    ...
    # obtain the integer
    f = (int(s_boxes[0][a]) + int(s_boxes[1][b])) % modulus
    f = f ^ int(s_boxes[2][c])
    f = f + int(s_boxes[3][d])
...
def initialize(key):
    ...
    for i in range(len(p_boxes)):
        value = (int(''.join(format(ord(i), 'b') for i in key[index % key_length])) << 24) + \
                (int(''.join(format(ord(i), 'b') for i in key[(index + 1) % key_length])) << 16) + \
                (int(''.join(format(ord(i), 'b') for i in key[(index + 2) % key_length])) << 8) + \
                 int(''.join(format(ord(i), 'b') for i in key[(index + 3) % key_length]))
        p_boxes[i] = p_boxes[i] ^ value
        index = index + 4

    left, right = 0, 0

    # loop goes from 0 to length of p-boxes, incrementing by 2

    for i in range(0, len(p_boxes), 2):
        left, right = cipher(left, right, 0)
        p_boxes[i] = left
        p_boxes[i + 1] = right
...
def decrypt(data):
    if not len(data) == 8:
        raise RuntimeError("Attempted to encrypt data of invalid block length: %s " % len(data))
    data_list = list(map(bin, str.encode(data)))
    cl = int(data_list[3][2:]) | (int(data_list[2][2:]) << 8) | (int(data_list[1][2:]) << 16) | (int(data_list[0][2:]) << 24)
    cr = int(data_list[7][2:]) | (int(data_list[6][2:]) << 8) | (int(data_list[5][2:]) << 16) | (int(data_list[4][2:]) << 24)

    xl, xr = cipher(cl, cr, 1)
    chars = bytes([
